{
  ...
  in {
    ...
    devShell = eachSystem (pkgs:
      pkgs.mkShell {
        buildInputs = [pkgs.uv pkgs.rsync (pkgs.python312.withPackages (p: [p.rich p.packaging p.toml p.urllib3 p.networkx]))];
      });
    packages = eachSystem (
      pkgs: {
        default = pkgs.python312Packages.buildPythonPackage {
          ...
          propagatedBuildInputs = [pkgs.uv] ++ (with pkgs.python312Packages; [rich packaging toml urllib3 networkx]);
        };
      }
    );
    apps = eachSystem (
